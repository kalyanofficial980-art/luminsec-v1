...
export async function startPassiveScan(formData: FormData) {
  ...
  if (!websiteId) {
    redirect("/app/websites?message=Website ID missing");
  }
  ...
  if (websiteError || !website) {
    redirect("/app/websites?message=Website not found");
  }
  ...
  if (jobError || !scanJob) {
    redirect(
      `/app/websites?message=${encodeURIComponent(
  ...
  try {
    ...
    redirect(`/app/websites?message=${encodeURIComponent(message)}`);
  }

  revalidatePath("/app");
  revalidatePath("/app/websites");
  revalidatePath(`/app/scans/${scanResultId}`);

  redirect(`/app/scans/${scanResultId}`);
}
