...
function riskClass(value: unknown) {
  ...
  if (risk.includes("critical")) return "border-red-400/30 bg-red-400/10 text-red-100";
  if (risk.includes("high")) return "border-orange-400/30 bg-orange-400/10 text-orange-100";
  if (risk.includes("medium") || risk.includes("moderate")) return "border-amber-400/30 bg-amber-400/10 text-amber-100";
  if (risk.includes("low")) return "border-emerald-400/30 bg-emerald-400/10 text-emerald-100";
          ...
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-start">
            <div>
              ...
              <p className="mt-4 max-w-3xl leading-8 text-slate-300">
                Review passive website security posture reports, scores, findings, and fix-first action plans.
              </p>
            </div>
        ...
        {error ? (
          ...
          <section className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-8 text-center">
            ...
            <p className="mx-auto mt-3 max-w-2xl leading-8 text-slate-300">
              Add a website and run your first passive security posture scan to generate a report.
            </p>
                  ...
                  <div className="flex flex-col justify-between gap-6 xl:flex-row xl:items-center">
                    <div className="min-w-0">
                      <div className="mb-3 flex flex-wrap gap-2">
                        <span className={`rounded-full border px-3 py-1 text-xs font-black ${riskClass(scan.risk_level)}`}>
                    ...
                    <div className="grid gap-4 sm:grid-cols-4 xl:w-[520px]">
                      <div className="rounded-2xl border border-white/10 bg-slate-950 p-4 text-center">
                        <p className="text-xs font-bold text-slate-500">Overall</p>
                        <p className={`mt-2 text-3xl font-black ${scoreClass(overallScore)}`}>
                          {overallScore}
                        </p>
                      </div>
                      ...
                      <div className="rounded-2xl border border-white/10 bg-slate-950 p-4 text-center">
                        ...
                        <p className="mt-2 text-2xl font-black text-white">{securityScore}</p>
                      </div>

                      <div className="rounded-2xl border border-white/10 bg-slate-950 p-4 text-center">
                        <p className="text-xs font-bold text-slate-500">Privacy</p>
                        <p className="mt-2 text-2xl font-black text-white">{privacyScore}</p>
                      </div>

                      <div className="rounded-2xl border border-white/10 bg-slate-950 p-4 text-center">
                        <p className="text-xs font-bold text-slate-500">Trust</p>
                        <p className="mt-2 text-2xl font-black text-white">{trustScore}</p>
                      </div>
                    </div>
                  </div>
