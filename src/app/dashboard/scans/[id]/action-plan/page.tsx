...
function priorityClass(priority: string) {
  if (priority === "fix_now")
    return "border-red-400/30 bg-red-400/10 text-red-100";
  if (priority === "fix_this_week")
    return "border-amber-400/30 bg-amber-400/10 text-amber-100";
  if (priority === "ask_developer")
...
function whatWeFound(finding: FindingRow) {
  return (
    ...
    text(
      finding.description,
      "This website security posture item should be reviewed.",
    )
  );
}
...
function sortFindings(findings: FindingRow[]) {
  ...
  return [...findings].sort((a, b) => {
    ...
    return (
      severityOrder[normalizeRisk(a.severity)] -
      severityOrder[normalizeRisk(b.severity)]
    );
  });
}
            ...
            return (
              ...
              >
                <div className="mb-2 flex flex-wrap gap-2">
                  <span
                    className={`rounded-full border px-3 py-1 text-xs font-black ${priorityClass(priority)}`}
                  >
...
export default async function ActionPlanPage({ params }: PageProps) {
  ...
  const fixNow = findings.filter(
    (finding) => getPriority(finding) === "fix_now",
  );
  const fixThisWeek = findings.filter(
    (finding) => getPriority(finding) === "fix_this_week",
  );
  const askDeveloper = findings.filter(
    (finding) => getPriority(finding) === "ask_developer",
  );
  const monitor = findings.filter((finding) =>
    ["monitor", "optional"].includes(getPriority(finding)),
  );

  const retestItems = findings
    .map((finding) => retestText(finding))
    .filter(Boolean);
      ...
      <div className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-col justify-between gap-8 xl:flex-row xl:items-start">
            <div>
              ...
              <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300">
                This fix-first action plan turns VeyraSec findings into owner
                actions, developer tasks, and retest steps.
              </p>
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-950 p-6 text-center">
              ...
              <div
                className={`mt-5 rounded-full border px-5 py-3 text-sm font-black ${severityClass(scan.risk_level)}`}
              >
        ...
        <section className="mt-8 rounded-3xl border border-cyan-300/20 bg-cyan-300/10 p-8">
          ...
          <p className="mt-4 leading-8 text-cyan-50/90">
            {riskReason ||
              "Fix the highest-priority findings first, then run a retest to confirm score improvement."}
          </p>

          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {(scoreImprovements.length > 0
              ? scoreImprovements
              : ["Fix highest-priority findings and retest the website."]
            ).map((item, index) => (
              <div
                key={`${item}-${index}`}
                className="rounded-2xl border border-cyan-200/20 bg-slate-950/60 p-4"
              >
          ...
          {findings.length > 0 ? (
            ...
            <div className="rounded-3xl border border-emerald-300/20 bg-emerald-300/10 p-6">
              ...
              <p className="mt-3 leading-7 text-emerald-50/90">
                This scan did not store visible findings that require an action
                plan.
              </p>
            </div>
          )}
          ...
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="mt-4 max-w-4xl leading-8 text-amber-50/90">
                This action plan is based on passive visible website signals. It
                is designed for business owners and developers to prioritize
                fixes. It is not exploit testing, login testing, brute force
                testing, compliance certification, or a penetration test.
              </p>
            </div>
          </div>
        </section>
