...
function statusClass(status: unknown) {
  ...
  if (value === "approved") return "border-emerald-400/30 bg-emerald-400/10 text-emerald-100";
  if (value === "rejected") return "border-red-400/30 bg-red-400/10 text-red-100";
...
function severityClass(value: unknown) {
  ...
  if (severity.includes("critical")) return "border-red-400/30 bg-red-400/10 text-red-100";
  if (severity.includes("high")) return "border-orange-400/30 bg-orange-400/10 text-orange-100";
  if (severity.includes("medium")) return "border-amber-400/30 bg-amber-400/10 text-amber-100";
  if (severity.includes("low")) return "border-emerald-400/30 bg-emerald-400/10 text-emerald-100";

  return "border-slate-400/30 bg-slate-400/10 text-slate-200";
}

export default async function ManualReviewQueuePage({ searchParams }: PageProps) {
  const params = await searchParams;
  const { supabase } = await requireAdmin();

  const requestedStatus = ["pending", "approved", "rejected"].includes(normalizeText(params.status).toLowerCase())
  ...
  if (requestedStatus === "pending") {
    findingsQuery = findingsQuery.or("manual_review_status.eq.pending,manual_review_status.is.null");
  } else {
  ...
  const rows = findings ?? [];
  const scanIds = Array.from(new Set(rows.map((finding) => normalizeText(finding.scan_result_id)).filter(Boolean)));

  const { data: scans } = scanIds.length > 0
    ? await supabase
        .from("scan_results")
        .select("id, url, domain, overall_score, score, risk_level, created_at")
        .in("id", scanIds)
    : { data: [] };
  ...
  const counts = {
    visible: rows.length,
    approved: rows.filter((finding) => finding.manual_review_status === "approved").length,
    rejected: rows.filter((finding) => finding.manual_review_status === "rejected").length,
    pending: rows.filter((finding) => !finding.manual_review_status || finding.manual_review_status === "pending").length,
  };
    ...
    <main className="min-h-screen bg-slate-950 p-6 text-white">
      <div className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-start">
            <div>
              ...
              <p className="mt-4 max-w-3xl leading-8 text-slate-300">
                Admin-only queue for reviewing findings before a paid report is treated as verified.
                Approve only findings with clear evidence. Reject noisy or incorrect findings.
              </p>
            </div>
        ...
        <section className="mt-8 grid gap-6">
          {rows.length > 0 ? (
            rows.map((finding, index) => {
              ...
              const status = normalizeText(finding.manual_review_status, "pending");
              const reportHref = finding.scan_result_id ? `/dashboard/scans/${finding.scan_result_id}` : "/dashboard/scans";

              return (
                <article key={finding.id} className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
                  <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-400">
                        {normalizeText(scan?.domain || scan?.url, "Unknown website")}
                      </p>
                    </div>
                  ...
                  <div className="mt-5 grid gap-4 lg:grid-cols-2">
                    <div className="rounded-2xl border border-white/10 bg-slate-950 p-4">
                      <h3 className="font-black text-cyan-100">Finding evidence</h3>
                      <p className="mt-3 whitespace-pre-wrap break-words text-sm leading-7 text-slate-300">
                        {normalizeText(finding.evidence, "No evidence stored. Review carefully before approving.")}
                      </p>
                    </div>

                    <div className="rounded-2xl border border-white/10 bg-slate-950 p-4">
                      <h3 className="font-black text-cyan-100">Recommendation</h3>
                      <p className="mt-3 whitespace-pre-wrap break-words text-sm leading-7 text-slate-300">
                        {normalizeText(finding.recommendation, "No recommendation stored.")}
                      </p>
                    </div>
                  </div>

                  <form action={updateFindingManualReviewAction} className="mt-5 rounded-2xl border border-white/10 bg-slate-950 p-4">
                    <input type="hidden" name="finding_id" value={finding.id} />

                    <label className="text-sm font-black text-slate-300" htmlFor={`status-${finding.id}`}>
                      Review decision
                    </label>
                    ...
                    <label className="mt-4 block text-sm font-black text-slate-300" htmlFor={`notes-${finding.id}`}>
                      Reviewer notes
                    </label>
        ...
        <section className="mt-8 rounded-3xl border border-amber-300/20 bg-amber-300/10 p-8">
          <h2 className="text-2xl font-black text-amber-100">Manual review rule</h2>
          <p className="mt-4 max-w-4xl leading-8 text-amber-50/90">
            Approved findings can be used for verified paid reports. Pending and rejected findings should not be presented as verified evidence.
            This review does not create legal certification, compliance proof, exploit testing, or a penetration test.
          </p>
        </section>
      </div>
    </main>
