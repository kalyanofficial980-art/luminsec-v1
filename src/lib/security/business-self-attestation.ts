export type SelfAttestationSeverity = "info" | "low" | "medium" | "high" | "critical";
...
export type BusinessSelfAttestationInput = {
  ...
  websiteUrl?: string;
  answeredBy?: string;
  answers: BusinessSelfAttestationAnswer[] | Record<string, SelfAttestationAnswerValue | boolean | string>;
};
...
export const BUSINESS_SELF_ATTESTATION_QUESTIONS: BusinessSelfAttestationQuestion[] = [
  {
    ...
    helpText: "Business knows what personal/customer data is collected through website, forms, payments, bookings, and support.",
    weight: 10,
    required: true,
    riskWhenNo: "medium",
  ...
  },
  {
    ...
    helpText: "Website has a privacy notice/policy explaining what data is collected and why.",
    weight: 10,
    required: true,
    riskWhenNo: "medium",
  ...
  },
  {
    ...
    helpText: "Customers can understand why their data is collected before or around data collection points.",
    weight: 8,
    required: true,
    riskWhenNo: "medium",
  ...
  },
  {
    ...
    helpText: "Important forms, marketing, tracking, or sensitive collection have clear notice/consent wording.",
    weight: 8,
    required: true,
    riskWhenNo: "medium",
  ...
  },
  {
    ...
    helpText: "Customers have a clear contact path for privacy, correction, deletion, or complaint requests.",
    weight: 8,
    required: true,
    riskWhenNo: "medium",
    evidenceExpectation: "Email, contact page, grievance contact, or support path.",
  },
  {
    id: "retention_policy_defined",
    category: "data_retention",
    label: "Data retention rules are defined",
    helpText: "Business knows how long customer data is retained and when it is deleted.",
    weight: 7,
    required: true,
    riskWhenNo: "medium",
  ...
  },
  {
    ...
    helpText: "Business can handle customer requests for correction, access, deletion, or withdrawal where applicable.",
    weight: 7,
    required: true,
    riskWhenNo: "medium",
  ...
  },
  {
    ...
    helpText: "Only required staff can access customer data, admin dashboards, CRM, billing, or orders.",
    weight: 8,
    required: true,
    riskWhenNo: "high",
  ...
  },
  {
    ...
    helpText: "Admin email, hosting, database, payment, CRM, and CMS accounts use MFA where possible.",
    weight: 8,
    required: true,
    riskWhenNo: "high",
  ...
  },
  {
    ...
    helpText: "Business knows third parties receiving customer data, such as hosting, analytics, payments, CRM, email, or WhatsApp tools.",
    weight: 7,
    required: true,
    riskWhenNo: "medium",
  ...
  },
  {
    ...
    helpText: "Business knows who handles suspected data leaks, website compromise, or security incidents.",
    weight: 8,
    required: true,
    riskWhenNo: "high",
  ...
  },
  {
    ...
    helpText: "Business has a basic process for identifying, recording, escalating, and reporting security incidents where required.",
    weight: 8,
    required: true,
    riskWhenNo: "high",
  ...
  },
  {
    ...
    helpText: "Business can access hosting, application, auth, payment, or admin activity logs for investigation.",
    weight: 7,
    required: true,
    riskWhenNo: "medium",
  ...
  },
  {
    ...
    helpText: "Business can recover website, database, orders, or CRM data after failure or compromise.",
    weight: 7,
    required: true,
    riskWhenNo: "medium",
...
function normalizeAnswer(value: unknown, allowNotApplicable?: boolean): SelfAttestationAnswerValue {
  if (typeof value === "boolean") return value ? "yes" : "no";

  const normalized = String(value || "").trim().toLowerCase();

  if (["yes", "y", "true", "done", "available"].includes(normalized)) return "yes";
  if (["partial", "partly", "in_progress", "progress", "some"].includes(normalized)) return "partial";
  if (["no", "n", "false", "missing", "not_done"].includes(normalized)) return "no";
  if (["unknown", "not sure", "na", ""].includes(normalized)) return "unknown";
  if (allowNotApplicable && ["not_applicable", "not applicable", "n/a"].includes(normalized)) return "not_applicable";

  return "unknown";
}

function answerScore(value: SelfAttestationAnswerValue, allowNotApplicable?: boolean) {
...
function severityForAnswer(question: BusinessSelfAttestationQuestion, value: SelfAttestationAnswerValue): SelfAttestationSeverity {
  if (value === "partial") return question.riskWhenNo === "high" ? "medium" : "low";
...
function findingForQuestion(
  ...
  return {
    ...
    expected_value: "yes",
    limitation: "Self-attested answer only. Manual review is required for verified paid reports.",
    root_cause: `self_attestation_${question.category}`,
  };
}

function emptyCategoryTotals() {
  const totals: Record<SelfAttestationCategory, { earned: number; possible: number }> = {
...
export function evaluateBusinessSelfAttestation(input: BusinessSelfAttestationInput): BusinessSelfAttestationScore {
  ...
  for (const question of BUSINESS_SELF_ATTESTATION_QUESTIONS) {
    ...
    const normalizedValue = normalizeAnswer(answer?.value, question.allowNotApplicable);
    ...
    if (normalizedValue === "partial" || normalizedValue === "no" || normalizedValue === "unknown") {
      findings.push(findingForQuestion(input, question, normalizedValue, answer));
    }
  }

  const categories = CATEGORY_ORDER.reduce((acc, category) => {
    acc[category] = percent(categoryTotals[category].earned, categoryTotals[category].possible);
    return acc;
...
export function businessSelfAttestationIsComplete(input: BusinessSelfAttestationInput) {
  return evaluateBusinessSelfAttestation(input).missingRequiredQuestionIds.length === 0;
}