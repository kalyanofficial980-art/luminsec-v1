...
export function compareScores(
  ...
  return {
    ...
    securityChange:
      Number(current.security_score ?? 0) -
      Number(previous.security_score ?? 0),
    privacyChange:
      Number(current.privacy_score ?? 0) - Number(previous.privacy_score ?? 0),
    trustChange:
      Number(current.trust_score ?? 0) - Number(previous.trust_score ?? 0),
  };
}
...
export function compareFindings(
  ...
  const unchangedFindings = currentFindings.filter((finding) =>
    previousMap.has(normalizeTitle(finding.title)),
  );
